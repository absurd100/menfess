...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("ban_"):
        ...
        except Exception as e: await query.answer(f"gagal menghapus clone")
...
#MENU OWNER
    if uid_int == MAIN_OWNER_ID:
        if text == '🇮🇩 ADMIN MENU':
            return await msg.reply_text("kembali ke mode owner", reply_markup=OWNER_KEYBOARD)
        if text == '📋 LIST CLONE':
            clones = load_json(CLONE_DB)
    ...
    if uid_int == OWNER_ID:
        if text == '🇮🇩 ADMIN MENU':
            return await msg.reply_text("kembali ke mode owner", reply_markup=CLONE_ADMIN_KEYBOARD)
    ...
    if context.user_data.get('waiting_clone'):
        ...
        except Exception as e: return await msg.reply_text(f"❌ Gagal cloning periksa token anda")
 ...
 # Capture Token Clone (PID SYSTEM)
    ...
    if not text.startswith('/') and text not in list_tombol:
        ...
        except Exception as e: await msg.reply_text(f"mohon periksa kembali")
