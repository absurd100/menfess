...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("ban_"):
        ...
        # Mengembalikan isi config ke DEFAULT_TEMPLATE yang ada di awal script
        cfg["post_template"] = DEFAULT_TEMPLATE
        save_json(CONFIG_FILE, cfg)
        
        # Memberikan notifikasi pop-up di Telegram
        await query.answer("✅ Template berhasil direset!", show_alert=True)
        
        # Opsional: Mengirim pesan konfirmasi di chat
        await query.message.reply_text(
            "✨ <b>RESET BERHASIL!</b>\n"
            "Template menfess telah dikembalikan ke pengaturan awal.",
        ...
        try:
            ...
            if 0 <= idx < len(clones):
                ...
                username_bot = removed.get('username', 'Unknown')
                ...
                await query.edit_message_text(
                    f"✅ Bot <b>@{username_bot}</b> berhasil dimatikan dan dihapus.", 
                    parse_mode=ParseMode.HTML
                )
            else: 
                await query.answer("Gagal: Index tidak ditemukan")
        except Exception as e: 
            await query.answer(f"Gagal menghapus clone")
...
#MENU OWNER
    if uid_int == MAIN_OWNER_ID:
        if text == '🇮🇩 ADMIN MENU':
            return await msg.reply_text("<i>kembali ke mode owner</i>", reply_markup=OWNER_KEYBOARD)

    if text == '📋 LIST CLONE':
        clones = load_json(CLONE_DB)
        if not clones: 
            return await msg.reply_text("Belum ada bot yang dikloning.")
        
        txt = "📋 <b>DAFTAR BOT CLONE</b>\n\n"
        kb = []
        for i, c in enumerate(clones):
            name = c.get('name', 'Bot Lama')
            user = c.get('username', '???')
            txt += f"{i+1}. <b>{name}</b> (@{user})\n"
            kb.append([InlineKeyboardButton(f"❌ Matikan @{user}", callback_data=f"delclone_{i}")])
        
        return await msg.reply_text(txt, reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.HTML)

    if uid_int == OWNER_ID:
        if text == '🇮🇩 ADMIN MENU':
            return await msg.reply_text("<i>kembali ke mode owner</i>", reply_markup=CLONE_ADMIN_KEYBOARD)  
            target_sekarang = cfg.get("target_channel", DEFAULT_CHANNEL)
            return await msg.reply_text(f"⚙️ <b>CUSTOM POST</b>\n\n"f"📢 Channel: <code>{target_sekarang}</code>\n"f"Silakan pilih menu untuk mengatur tampilan menfess.", reply_markup=kb, parse_mode=ParseMode.HTML
        )
    ...
    if context.user_data.get('waiting_clone'):
        ...
        try:
            ...
            return await msg.reply_text(f"❌ Gagal: Token tidak valid atau error sistem.")
 ...
 # Capture Token Clone (PID SYSTEM)
    if context.user_data.get('waiting_clone'):
        ...
        except Exception as e: return await msg.reply_text(f"❌ Gagal: error")
    ...
    if not text.startswith('/') and text not in list_tombol:
        ...
        # --- PERBAIKAN 1: Ambil info bot secara real-time ---
        ...
        # --- PERBAIKAN 2: Gunakan bot_info yang sudah didefinisikan ---
        ...
        # Susun template dengan mengganti {BOT_LINK}
        ...
        try:
            if msg.photo: 
                # Untuk foto, link preview biasanya tidak muncul, tapi tetap bisa ditambahkan jika perlu
                ...
                # UNTUK TEKS: Tambahkan link_preview_options untuk mematikan webview
                snt = await context.bot.send_message(
                    target, 
                    text=final_text, 
                    parse_mode=ParseMode.HTML,
                    link_preview_options=LinkPreviewOptions(is_disabled=True) # MEMATIKAN PREVIEW
                )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"👋 Halo! {full_name}\nID: {uid_int}\nselamat datang di bot menfess canggih 😎\nkamu bisa pilih mode anonim atau terlihat\n\n<blockquote>created by Ano</blockquote>", reply_markup=kb)

def main():
    pengaturan_default = Defaults(parse_mode=ParseMode.HTML)
    app = Application.builder().token(TOKEN).defaults(pengaturan_default).build()
    if IS_CLONE:
        clones = load_json(CLONE_DB)
